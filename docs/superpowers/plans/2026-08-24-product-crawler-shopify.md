...
| `db/migrations/0017_*.sql` | Generated by drizzle-kit. Nothing hand-written. |
...
- Create: `db/migrations/0017_*.sql` (generated)
...
Expected: a new file `db/migrations/0017_<name>.sql` containing the enum changes. Read it and confirm it does not drop or rewrite the `job` table:

```bash
cat db/migrations/0017_*.sql
```

Expected: only `ALTER TABLE … DROP CONSTRAINT … / ADD CONSTRAINT … CHECK` statements against `job` and `job_log`. If it contains a `DROP TABLE`, stop and report it — that would destroy run history.
...
```bash
./tests/crawl.sh
git add lib/crawl-options.ts db/schema.ts db/migrations lib/jobs.ts lib/job-display.ts 'app/api/jobs/[id]/schedule/route.ts' 'app/api/jobs/[id]/retry-failed/route.ts'
